...
struct P {
  ...
  std::unique_ptr<Expr> symbol() {
    ws();
    if (i >= s.size() || !std::isalpha((unsigned char)s[i])) throw std::runtime_error("expected symbol");
    std::string name;
    while (i < s.size() && (std::isalnum((unsigned char)s[i]) || s[i] == '_')) name.push_back(s[i++]);
    return std::make_unique<Symbol>(name);
  }

  std::unique_ptr<Expr> latex_frac() {
    i += 5; // \frac
    P np(brace_content());
    P dp(brace_content());
  ...
  std::unique_ptr<Expr> latex_sqrt() {
    i += 5; // \sqrt
    P rp(brace_content());
    return std::make_unique<Sqrt>(rp.expr());
  }
  ...
  std::unique_ptr<Expr> atom() {
    ...
    if (i < s.size() && s[i] == '\\') {
      if (s.compare(i, 5, "\\frac") == 0) return latex_frac();
      if (s.compare(i, 5, "\\sqrt") == 0) return latex_sqrt();
      throw std::runtime_error("unknown latex command");
    }
  ...
  std::unique_ptr<Expr> postfix() {
    ...
    while (true) {
      ...
      if (eat('^')) {
        ...
        base = std::make_unique<Superscript>(std::move(base), std::move(sup));
      } else if (eat('_')) {
        ...
        base = std::make_unique<Subscript>(std::move(base), std::move(sub));
      } else {
        break;
      }
    }
