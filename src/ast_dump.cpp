...
std::string ast_to_json(const Expr& expr) {
  ...
  if (auto d = dynamic_cast<const Decimal*>(&expr)) { o << "{\"type\":\"Decimal\",\"text\":\"" << d->text << "\"}"; return o.str(); }
  if (auto s = dynamic_cast<const Symbol*>(&expr)) { o << "{\"type\":\"Symbol\",\"name\":\"" << s->name << "\"}"; return o.str(); }
  if (auto fn = dynamic_cast<const FunctionCall*>(&expr)) { o << "{\"type\":\"FunctionCall\",\"name\":\"" << fn->name << "\",\"arg\":" << ast_to_json(*fn->arg) << "}"; return o.str(); }
  ...
  if (auto q = dynamic_cast<const Sqrt*>(&expr)) {
    o << "{\"type\":\"Sqrt\"";
    if (q->index) o << ",\"index\":" << ast_to_json(*q->index);
    o << ",\"radicand\":" << ast_to_json(*q->radicand) << "}";
    return o.str();
  }
